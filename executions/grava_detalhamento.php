        ...
        <div id="page-wrapper">
            <div class="header"> 
                <h1 class="page-header">
                    Adicionar Detalhamento de Levantamento para Obra
                </h1>
                <ol class="breadcrumb">
     						...
     						<div class="table-responsive">
                                <?php
    								...
    								if(!empty($_POST)){
    									...
    									if(isset($_POST['data']) && isset($_POST['local']) && isset($_POST['id_os'])){

    										if(!empty($_POST['data']) && !empty($_POST['local']) && !empty($_POST['id_os'])){

    											$id_os = $_POST['id_os'];
                                                $data = $_POST['data'];
                                                $local = str_replace("'","",$_POST['local']);

                                                $observation = "<br><b>Local: </b>".$local."<br>" ;
    											...
    											if (!$link){
    											  ...
    											  die('Connect Error (' . mysqli_connecterrno() . ')' .
                                                    ...
                                                    $query_insert_detalhamento = "INSERT INTO `detalhamentos`(`id_detalhamento`, `dt_detalhamento`, `fk_id_os_obra`, `detalhamento`) VALUES (NULL,'$data','$id_os','$relato')";
                                                    $result_insert_detalhamento = $link->query($query_insert_detalhamento) or die (  mysqli_error( $link ) );

                                                    if ($result_insert_detalhamento == TRUE) {
                                                        ...
                                                        $query_add_log = "INSERT INTO `log_acoes`(`id_acao`, `data_acao`, `descricao_acao`, `fk_id_user`) VALUES (NULL,'$DT_atual','Detalhamento de Levantamento para Obra inserido na OS: $protocolo','$id_usersede')";
                                                        ...
                                                        //###################################################################################################################
    														...
    														<div class="input-field col s12">
    											                <div class="alert alert-danger">
    											                    <strong>A Levantamento não foi registrado. Tente novamente.</strong>
    											                </div>            
    											            </div>
