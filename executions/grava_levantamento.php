     						...
     						<div class="table-responsive">
                                <?php
    								...
    								if(!empty($_POST)){
    									...
    									if(isset($_POST['data']) && isset($_POST['levantamento']) && isset($_POST['id_os']) && $_POST['levantamento'][0] !== '' && $_POST['item'][0] !== ''){

    										if(!empty($_POST['data']) && !empty($_POST['levantamento']) && !empty($_POST['id_os'])){
                                                ...
                                                $date = "<b>Data do Levantamento: </b>".$data;
                                                $observation = "<br><b>Locais: </b><br><br>".str_replace("\n",'<br />', $text_levantamento);
                            
                                                $relato = "{$date}{$observation}";

                                                /**for ($i=0; $i < count($array_itens); $i++) { 
                                                    echo nl2br($array_itens[$i].' - '.$array_qtdes[$i].' - '.$array_refs[$i].' - '.$array_setores[$i].'<br>');
                                                }**/
                                                ...
                                                if (!$link){
                                                  ...
                                                  die('Connect Error (' . mysqli_connecterrno() . ')' .
                                                    ...
                                                    $query_update_os = "UPDATE `ordem_servico` SET `status`='$status' WHERE `id_os` = '$id_os'";
                                                    $result_update_os = $link->query($query_update_os) or die(  mysqli_error( $link ) );

                                                    if($result_update_os == TRUE){

                                                        $query_insert_cotacao = "INSERT INTO `cotacoes_os`(`id_cotacao`, `status_cotacao`, `dt_cotacao`, `fk_id_os_cotada`, `obs_cotacao`) VALUES (NULL,'0','$data','$id_os','$relato')";
                                                        ...
                                                        $query_add_log = "INSERT INTO `log_acoes`(`id_acao`, `data_acao`, `descricao_acao`, `fk_id_user`) VALUES (NULL,'$DT_atual','Levantamento inserido na OS: $protocolo','$id_usersede')";
                                                        ...
                                                        //###################################################################################################################
                                                            ...
                                                            <div class="input-field col s12">
                                                                <div class="alert alert-success">
                                                                    <strong>Levantamento Registrado com Sucesso!</strong>
                                                                </div>
                                                            ...
                                                            <div class="input-field col s12">
                                                                <div class="alert alert-danger">
                                                                    <strong>O Levantamento não foi registrado. Tente novamente.</strong>
                                                                </div>            
                                                            </div>
