    								...
    								if(!empty($_POST)){
    									...
    									if(isset($_POST['id_escola'])){
    										...
    										if(!empty($_POST['id_escola'])){
                                                ...
                                                $query_edit_infra = "UPDATE `infraestrutura` SET `mnt_preventiva`='$MP',`vistoria`='$VT',`internet`='$I',`infra_rede`='$IR',`lab_info`='$LI',`biometria`='$B',`camera`='$CA',`saber`='$SA' WHERE fk_id_escola ='$id_escola'";
    											...
    											if (!$link){
    											  ...
    											  die('Connect Error (' . mysqli_connecterrno() . ')' .
                                                    ...
                                                    if(!empty($_POST['serial'])){
                                                        ...
                                                        $query_serial_b = "SELECT * from escola where id_escola = '$escola'";
                                                        $row_serial_b = mysqli_fetch_object($link->query($query_serial_b));
                                                        $serial_b_b = $row_serial_b->serial_b;

                                                        if ($serial_b_b != $serial) {
                                                            $query_serial = "UPDATE `escola` SET `serial_b` = '$serial' WHERE id_escola = '$escola'";
                                                            $result_serial = $link->query($query_serial) or die( mysqli_error( $link));

                                                            $query_history_serial = "INSERT INTO `history_serial`(`id_history_serial`, `data_mudanca`, `num_serial`, `fk_id_escola_serial`) VALUES (NULL,'$data','$serial','$escola')";
                                                            $result_history_serial = $link->query($query_history_serial) or die( mysqli_error( $link));
                                                        }

                                                        $query_serial = "UPDATE `escola` SET `serial_b` = '$serial' WHERE id_escola = '$id_escola'"; 
                                                        $result_serial = $link->query($query_serial) or die( mysqli_error( $link));
                                                    } 
