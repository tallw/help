...
$arquivo = 'Relatorio SLA por Sede(';
...
if($dt_inicial != "" && $dt_final != "") {
    ...
    foreach ($meses_index as $key => $value) {
		...
		for ($i=1; $i < 4; $i++) { 
			...
			$Num_O_S = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and (O.dt_conclusao BETWEEN '$dt_inicial' and '$dt_final') and O.fk_id_sede = '$i'  and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val'"));

			$PROGRAM = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and (O.dt_conclusao BETWEEN '$dt_inicial' and '$dt_final') and O.fk_id_sede = '$i' and O.tipo_chamado = 1 and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val'"));

			$CHAMADOS =	mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and (O.dt_conclusao BETWEEN '$dt_inicial' and '$dt_final') and O.fk_id_sede = '$i' and O.tipo_chamado = 0 and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val'"));
			...
			$DD72_HRS_Atend_ok = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and (O.dt_conclusao BETWEEN '$dt_inicial' and '$dt_final') and O.fk_id_sede = '$i'  and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val' and O.sla_atendimento < 4 and O.tipo_chamado = 0"));

			$HRS72_Atend_off = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O  where O.status = 3 and (O.dt_conclusao BETWEEN '$dt_inicial' and '$dt_final') and O.fk_id_sede = '$i'  and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val' and O.sla_atendimento > 3 and O.tipo_chamado = 0"));

			$DD15_Resolucao_ok = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and O.fk_id_sede = '$i'  and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val' and O.sla_conclusao < 16 and O.tipo_chamado = 0"));

			$DD15_Resolucao_off = mysqli_num_rows(mysqli_query($link, "SELECT DISTINCT O.id_os from ordem_servico O where O.status = 3 and O.fk_id_sede = '$i'  and MONTH(O.dt_conclusao) = '$mes_val' and Year(O.dt_conclusao) = '$ano_val' and O.sla_conclusao > 15 and O.tipo_chamado = 0"));
